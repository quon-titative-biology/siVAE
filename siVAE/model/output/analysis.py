...
from . import output_handler as oh
...
def extract_value(result_dict):
    """ """
    result  = result_dict.get_model()
    if 'sample' not in result_dict.get_dict().keys():
        z_mu    = result['z_mu']
        z_var   = result['y_mu']
    ...
    if 'sample_dict' in result.get_dict().keys():
    ...
    # Sample-wise VAE
    if 'sample' in result_dict.get_dict().keys():
        result_sample = result_dict.get_dict()['sample']
        if result_sample is not None:
            ...
            values_dict['sample'] = {'z_mu' : z_mu,
                                     'z_var': z_var}
    # Feature-wise VAE
    if 'feature' in result_dict.get_dict().keys():
        result_feature = result_dict.get_dict()['feature']
...
def infer_FA_loadings(scores,method=1):
    """
    ...
    for scores_FA in scores:
        ...
        for scores_LE in scores_FA:
            if (scores_LE[0] == scores_LE).all():
                FA_loadings = scores_LE[0]
            else:
                pca = PCA(n_components = 1)
                if method==1:
                    pca.fit(scores_LE)
                    FA_loadings = (pca.components_.T * np.sqrt(pca.explained_variance_)).T[0]
                if method==2:
                    FA_loadings = pca.fit_transform(scores_LE.T).T[0]#
...
def plot_FA_loadings(siVAE_result,logdir,ImageDims):
    """ Plot the inferred FA loadings """
    scores_dict, method_DE = oh.get_feature_attributions(siVAE_result)
    for AE_part, scores in scores_dict.items():
        FA_loadings = infer_FA_loadings(np.swapaxes(scores,1,2))
        images_list = FA_loadings.reshape(*FA_loadings.shape[:-1],*ImageDims)
        colnames = ['Dimensions'] + method_DE
...
def plot_siVAE_loadings(siVAE_result, logdir, ImageDims,cutoff=0.1,**kwargs):
    loadings = siVAE_result.get_feature_embeddings().T
    images_list = [loadings.reshape(*loadings.shape[:-1],*ImageDims)]
    colnames = ['Dimensions','siVAE']
    rownames = ['Dim-'+str(dim+1) for dim in range(len(images_list[0]))]
    _ = plot_images(images_list,colnames=colnames,rownames=rownames,cutoff=cutoff,**kwargs)
    figname = os.path.join(logdir,"Loadings-siVAE.pdf")
    plt.savefig(figname)
    plt.close()


def plot_siVAE_pre_loadings(values_dict, logdir, ImageDims,cutoff=0.1,**kwargs):
    """ """
    loadings = values_dict['feature']['z_mu'].T
    images_list = [loadings.reshape(*loadings.shape[:-1],*ImageDims)]
    colnames = ['Dimensions','siVAE_pre']
    rownames = ['Dim-'+str(dim+1) for dim in range(len(images_list[0]))]
    _ = plot_images(images_list,colnames=colnames,rownames=rownames,cutoff=cutoff,**kwargs)
    figname = os.path.join(logdir,"Loadings-siVAE_pre.pdf")
    plt.savefig(figname)
    plt.close()
...
def save_losses(result_dict,logdir):
    ...
    losses = result_dict.get_value('losses')
    df = pd.DataFrame(losses)
    df.to_csv(os.path.join(logdir,'losses.csv'),sep=',',index=False,header=True)

    if 'sample' in result_dict.get_dict().keys():
        if result_dict.get_dict()['sample'] is not None:
            losses = result_dict.get_dict()['sample'].get_value('losses')
            df = pd.DataFrame(losses)
            df.to_csv(os.path.join(logdir,'losses_sample.csv'),sep=',',index=False,header=True)

    if 'feature' in result_dict.get_dict().keys():
        if result_dict.get_dict()['feature'] is not None:
            losses = result_dict.get_dict()['feature'].get_value('losses')
            df = pd.DataFrame(losses)
...
def plot_latent_embeddings(values_dict, plot_args_dict_sf, palette, method_dim_reds = ['PCA','tSNE'], logdir='', multidimension=False, show_legend=True,**kwargs):
    ...
    if 'v_mu' in values_dict['model'].keys():
...
def compare_loadings2(values_dict,logdir,loadings=None,loadings_name=None,**kwargs):
    #
    siVAE_loadings = values_dict['model']['v_mu'].T
    #
    dims = siVAE_loadings.shape[0]
    ...
    if dims < 5:
        #
        figname = os.path.join(logdir,'loadings_comparison_scatter_{}.pdf'.format(loadings_name))
        with PdfPages(figname) as pp:
            for i in range(dims):
                df = pd.DataFrame({'siVAE':siVAE_loadings[i],
                                   loadings_name:loadings[i]})
                ax = sns.scatterplot(data=df,x='siVAE', y=loadings_name,**kwargs)
                plt.title('siVAE vs {} - dim {}'.format(loadings_name,i+1))
                pp.savefig()
                plt.clf()
        plt.close()
...
def plot_feature_awareness(result_dict,scaler,ImageDims,plot_args_dict_sf,logdir):
    ...
    recon_loss_list = []
    x_recon_list = result_dict.get_model()['sample_dict']['output_hl'] + [x_recon]
